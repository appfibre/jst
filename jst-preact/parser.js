      ...
      const visitor = {   
        ArrayExpression: { enter(path) {   
          ...
          if (!t.isCallExpression(path.parent) ) {
              // var _parent = require('preact').CreateElement('...')
              var id = parent.scope.generateUidIdentifier("parent"); 
              parent.insertBefore(t.variableDeclaration("let", [t.variableDeclarator(id, t.callExpression(t.memberExpression(t.callExpression(t.Identifier('require'), [t.StringLiteral('preact')]), t.Identifier('h')), path.node.elements))]));
              path.replaceWith(id);
            }
          }
        }, 
      ...
      if (t.isStringLiteral(path.node.key) && path.node.key.value== "__jst" && t.isObjectExpression(path.parent)) {
        if (t.isObjectExpression(path.node.value)) {
          for (var control in path.node.value.properties)
            path.get('value.properties.' + control + '.value').replaceWith( t.classExpression(t.identifier(path.node.value.properties[control].key.value), t.memberExpression(t.callExpression(t.identifier('require'), [t.stringLiteral('preact')]), t.identifier('Component'))
            ...
            path.get('value').replaceWith( t.classExpression(t.identifier('_default'), t.memberExpression(t.callExpression(t.identifier('require'), [t.stringLiteral('preact')]), t.identifier('Component'))
